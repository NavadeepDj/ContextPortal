...
@patch("httpx.get")
def test_get_latest_version_non_200(mock_get):
    mock_response = MagicMock()
...
@patch("shutil.which")
def test_detect_install_manager_uv(mock_which, mock_run):
...
@patch("subprocess.run")
@patch("shutil.which")
def test_detect_install_manager_pipx(mock_which, mock_run):
    mock_which.side_effect = lambda cmd: (
    ...
    mock_proc.returncode = 0
    mock_proc.stdout = (
        "package contextportal 0.2.0, installed using Python 3.12"
    )
    mock_run.return_value = mock_proc
...
@patch("shutil.which")
def test_detect_install_manager_fallback_pip(mock_which):
...
@patch("subprocess.run")
def test_run_upgrade_success(mock_run):
    mock_proc = MagicMock()
    mock_proc.returncode = 0
...
@patch("subprocess.run")
def test_run_upgrade_failure(mock_run):
    mock_proc = MagicMock()
...
def test_run_update_already_up_to_date(mock_curr, mock_latest, capsys):
...
def test_run_update_check_only_new_version(
    mock_curr, mock_latest, mock_mgr, capsys
):
...
def test_run_update_auto_upgrade(
    mock_curr, mock_latest, mock_mgr, mock_upgrade, capsys
):
    mock_curr.return_value = "0.2.0"
    mock_latest.return_value = "0.3.0"
    mock_mgr.return_value = "uv"
